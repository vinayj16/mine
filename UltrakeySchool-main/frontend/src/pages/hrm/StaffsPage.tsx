...
const StaffsPage = () => {
  ...
  const exportColumns: ExportColumn[] = [
    { key: 'employeeId', label: 'Staff ID', format: (v, row) => v || row._id?.slice(-6) },
    { key: 'name', label: 'Name', format: (_, row) => `${row.firstName || ''} ${row.lastName || ''}` },
    { key: 'email', label: 'Email' },
    { key: 'phone', label: 'Phone' },
    { key: 'department', label: 'Department', format: (_, row) => row.departmentId?.name || row.department || 'N/A' },
    { key: 'designation', label: 'Designation' },
    { key: 'status', label: 'Status' },
    { key: 'joinDate', label: 'Date of Join', format: (v) => v ? new Date(v).toLocaleDateString() : 'N/A' },
  ];
  ...
  const handleExport = (type: 'pdf' | 'excel') => {
    ...
    if (type === 'pdf') {
      exportToPDF(data, 'staffs', exportColumns, 'Staff List');
    } else {
      exportToExcel(data, 'staffs', exportColumns);
    }
  };
