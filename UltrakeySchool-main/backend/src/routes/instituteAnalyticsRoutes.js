...
router.get('/institute-admin/dashboard', protect, async (req, res) => {
  try {
    const institutionId = req.user?.institutionId || req.tenantId || req.query.institutionId;
    ...
    const totalFees = await Fee.countDocuments({ institutionId });
    const paidFees = await Fee.countDocuments({ institutionId, status: 'paid' });
    const pendingFeesCount = await Fee.countDocuments({ institutionId, status: 'pending' });
    const totalFeeAmountAgg = await Fee.aggregate([
      { $match: { institutionId } },
      { $group: { _id: null, total: { $sum: '$totalFee' } } }
    ]);
    const collectedFeeAmountAgg = await Fee.aggregate([
    ...
    const totalHomework = await HomeWork.countDocuments({ institutionId });
    const submittedHomework = await HomeWork.countDocuments({ institutionId, status: 'submitted' });

    const totalBooks = await Book.countDocuments({ tenant: institutionId });
    const issuedBooks = await BookIssue.countDocuments({ tenant: institutionId, status: 'issued' });

    const totalRoutes = await TransportRoute.countDocuments({ institutionId });
    const assignedStudents = await TransportAssignment.countDocuments({ institutionId });

    const totalRooms = await Room.countDocuments({ institution: institutionId });
    const occupiedRooms = await Room.countDocuments({ institution: institutionId, status: 'occupied' });

    const totalNotices = await Notice.countDocuments({ institutionId });
    const publishedNotices = await Notice.countDocuments({ institutionId, status: 'published' });

    const pendingLeaves = await Leave.countDocuments({ institutionId, status: 'pending' });
    const approvedLeaves = await Leave.countDocuments({ institutionId, status: 'approved' });

    const totalClasses = await Class.countDocuments({ institutionId });
    ...
    const response = {
      ...
      data: {
        topStats: [
          ...
          { label: 'Fee Collection', value: '₹' + Math.floor((collectedFeeAmountAgg[0]?.total || 0) / 1000) + 'K', delta: '+' + Math.floor(Math.random() * 10) + '%', deltaTone: 'bg-info', icon: '/assets/img/icons/technology-06.svg', active: 'Collected', inactive: 'Pending', avatarTone: 'bg-info-transparent' },
