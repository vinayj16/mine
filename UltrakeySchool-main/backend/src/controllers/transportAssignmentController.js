...
const getAllAssignments = async (req, res) => {
  try {
    ...
    if (!institutionId) {
      logger.info('No institutionId provided for assignments');
    }
    ...
    let result = await transportAssignmentService.getAllAssignments(institutionId, filters);

    // Check if result is an array or an object with data array
    ...
    logger.info('Transport assignments fetched successfully:', { institutionId, count: finalDataArray.length });
    return successResponse(res, finalDataArray, 'Transport assignments retrieved successfully', finalPagination);
  } catch (error) {
