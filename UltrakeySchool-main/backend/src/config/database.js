...
const connectDB = async () => {
  try {
    ...
    // Clean up any remaining deprecated collections (non-blocking, safe to run every startup)
    const cleanupDeprecated = async (db) => {
      try {
        ...
        // Merge audit_logs into auditlogs (handle legacy naming)
        if (collectionNames.includes('audit_logs')) {
          const legacyDocs = await db.collection('audit_logs').find({}).toArray();
          if (legacyDocs.length > 0) {
            if (!collectionNames.includes('auditlogs')) {
              await db.createCollection('auditlogs');
            }
            for (const doc of legacyDocs) {
              doc.mergedFrom = 'audit_logs';
        ...
        // Merge adminactivities into auditlogs
        if (collectionNames.includes('adminactivities')) {
          const adminDocs = await db.collection('adminactivities').find({}).toArray();
          if (adminDocs.length > 0) {
            if (!collectionNames.includes('auditlogs')) {
              await db.createCollection('auditlogs');
            }
            for (const doc of adminDocs) {
              doc.mergedFrom = 'adminactivities';
              doc.category = doc.resourceType || 'other';
              await db.collection('auditlogs').updateOne(
                { _id: doc._id },
                { $set: doc },
                { upsert: true }
          ...
          await db.collection('adminactivities').drop();
        }
    ...
    cleanupDeprecated(conn.connection.db);
    ...
    connectionAttempts++;
    logger.error(`[DB] MongoDB connection error (attempt ${connectionAttempts}/${MAX_RETRY_ATTEMPTS}):`, error.message);
...
const setupEventHandlers = () => {
  ...
  mongoose.connection.on('fullsetup', () => {
    logger.info('[DB] Mongoose replica set fully connected');
  });
  ...
  mongoose.set('debug', (collectionName, method, query, doc, options) => {
    ...
    return () => {
      ...
      if (duration > 100) { // Log slow queries (>100ms)
        logger.warn(`[DB] Slow Query (${duration}ms):`, logData);
      } else {
...
export const createIndexes = async () => {
  try {
    logger.info('[DB] Creating database indexes...');
    ...
    const safeCreateIndex = async (collectionName, indexSpec, options) => {
      try {
        ...
        const skipIndexes = [
          ...
          { collection: 'students', field: 'institutionId' },
          { collection: 'teachers', field: 'institutionId' },
    ...
    logger.info('[DB] Database indexes created successfully');
  } catch (error) {
    logger.error('[DB] Error creating indexes:', error.message);
  }
};
